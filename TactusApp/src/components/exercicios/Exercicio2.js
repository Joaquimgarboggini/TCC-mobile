...
const Exercicio2 = () => {
  ...
  useEffect(() => {
    if (sustainedNotes && waitingForInput && !finished) {
      // Verificar se há novas notas tocadas
      for (const note of sustainedNotes) {
        if (!processedNotes.has(note)) {
          console.log('Nova nota detectada via ScaleContext:', note);
  ...
  }, [sustainedNotes, waitingForInput, finished, processedNotes]);
  ...
  useEffect(() => {
    console.log('Exercicio2 - Estados atuais:', {
      currentRound,
      pontuacao,
      sequenciaAcertos,
  ...
  const finishExercise = async () => {
    ...
    // Salvar pontuação
    await saveExerciseScore('Exercicio2', pontuacao, true);
  ...
  if (!scaleNotes || scaleNotes.length === 0) {
    return (
      <View style={styles.container}>
        <HeaderMinimal title="Memória Visual 1s" iconType="memoria" onBack={() => navigation.goBack()} />
        ...
        <View style={{
          ...
          {showingNote ? (
            <Text style={{
              ...
              {String(getNoteInPortuguese(targetNote) || 'N/A')}
            </Text>
          ) : lastResult === 'correct' ? (
            <Text style={{
              fontSize: 18,
          ...
          <View style={{
            ...
            <View style={{
              ...
              <Text style={{
                ...
                Memória Visual 1s{'\n\n'}
                Notas aleatórias irão aparecer na tela em diferentes posições por 1 segundo. Você deverá pressionar a nota correspondente utilizando a luva no menor tempo possível.{'\n\n'}
                Pontuação:{'\n'}
                • Acerto: +5 + sequência{'\n'}
                • Erro: -2
              </Text>
