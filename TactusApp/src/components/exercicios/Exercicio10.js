// Exercício 10 - Escala Descendente com Partitura (Leitura)
// Mostra as partituras da escala selecionada em ordem descendente, repetindo 2 vezes (16 notas total)
...
const Exercicio10 = () => {
  ...
  useEffect(() => {
    console.log('🔍 Exercicio10: useEffect triggered - sustainedNotes:', sustainedNotes, 'finished:', finished);
    
    if (sustainedNotes && sustainedNotes.size > 0 && !finished) {
      console.log('🎯 Exercicio10: Verificando notas sustentadas...', Array.from(sustainedNotes));
      
      // Para exercícios contínuos, processar imediatamente qualquer nota
      for (const note of sustainedNotes) {
        console.log('🎹 Exercicio10: Processando nota:', note, 'Target:', targetNote);
        handleKeyPress(note);
  ...
  // Gerar sequência descendente da escala (2 repetições)
  const generateDescendingSequence = () => {
    if (!scaleNotes || scaleNotes.length === 0) return [];
    
    // Ordenar as notas da escala em ordem descendente por frequência
    const sortedNotes = [...scaleNotes].sort((a, b) => {
      ...
      if (octaveA !== octaveB) return octaveB - octaveA; // Invertido para descendente
      return noteOrder.indexOf(noteB) - noteOrder.indexOf(noteA); // Invertido para descendente
    });
  ...
  useEffect(() => {
    if (scaleNotes && scaleNotes.length > 0) {
      const sequence = generateDescendingSequence();
      setScaleSequence(sequence);
      console.log('Sequência descendente gerada:', sequence);
    }
  }, [scaleNotes]);
  ...
  const handleKeyPress = (pressedNote) => {
    ...
    console.log('🎯 Exercicio10: Tecla pressionada:', pressedNote, 'Nota alvo:', targetNote);
    ...
    if (pressedNote === targetNote) {
      ...
      console.log('✅ Exercicio10: Resposta correta! Pontos:', points, 'Sequência:', sequenciaAcertos + 1);
    } else {
      // Resposta incorreta
      ...
      console.log('❌ Exercicio10: Resposta incorreta! Pontuação:', pontuacao - 2);
    }
  ...
  const finishExercise = async () => {
    ...
    // Salvar pontuação
    await saveExerciseScore('Exercicio10', pontuacao, true);
          ...
          <View style={{
            ...
            <View style={{
              ...
              <Text style={{
                ...
                Escala Descendente com Partitura 16{'\n\n'}
                Você verá cada nota da escala selecionada em ordem descendente (do agudo ao grave) na forma de partitura. A sequência se repete 2 vezes, totalizando 16 notas.{'\n\n'}
